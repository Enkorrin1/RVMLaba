...
const stageMeta = {
  wake: {
    label: "Жилая комната маяка",
    objective: "Цель: осмотреть комнату и вспомнить, как шторм занёс тебя в маяк.",
  },
  lantern: {
    label: "Фонарный ярус",
  ...
  shore: {
    label: "Башня",
    objective: "Цель: осмотреть маяк и понять, кто принёс тебя сюда после шторма.",
  },
  lighthouse: {
    label: "Маяк",
  ...
  service: {
    ...
    objective: "Цель: осмотреть нижние помещения и найти новые зацепки о маяке.",
  },
  tunnel: {
    label: "Восточный тоннель",
    objective: "Цель: пройти по восточному тоннелю и понять, кто ходил здесь до тебя.",
  },
  pier: {
    label: "Нижняя пристань",
    objective: "Цель: осмотреть нижнюю пристань и собрать следы недавнего присутствия.",
  },
  bay: {
    label: "Скрытая бухта",
    objective: "Цель: исследовать скрытую бухту и найти, что скрывают скалы у маяка.",
  },
};
...
const gameState = {
  lastTime: 0,
  worldTime: 0,
  activeMessage: "Голова гудит после шторма. Последнее, что помнится, это маленькая лодка, волны и темнота.",
...
const inventoryCatalog = {
  ...
  serviceKey: {
    ...
    description: "Потемневший служебный ключ с жетоном маяка. Он должен открывать створки внутри тоннеля.",
...
const interactables = [
  {
    ...
    onInteract() {
      return {
        text: "Одеяло сырое и солёное. Похоже, тебя втащили сюда прямо после шторма, когда ты был без сознания.",
        objective: "Цель: осмотреть комнату и найти следы того, кто спас тебя с моря.",
      };
    },
  },
  {
    ...
    onInteract() {
      ...
      return {
        text: isContainerEmpty("clutter")
          ? "Ты выдвигаешь ящик тяжёлого стола у стены. Внутри уже пусто."
  ...
  },
  {
    ...
    onInteract() {
      if (!gameState.wakeProgress.cluesChecked) {
        return {
          text: "Рука уже на двери, но уходить вслепую рано. Сначала стоит понять, кто вытащил тебя из моря и оставил здесь.",
      ...
      startSceneTransition(
        "Основание маяка",
        "Жилая комната остаётся позади. Ветер сразу бьёт в лицо, а башня теперь ощущается как место, где ты пришёл в себя после шторма."
      );
  ...
  },
  {
    ...
    onInteract() {
      ...
      startSceneTransition(
        "Жилая комната маяка",
        "Ты снова входишь в жилую комнату маяка. Отсюда всё началось: мокрая постель, чужие вещи и лестница наверх."
      );
      return {
        text: "Наружная дверь снова ведёт внутрь маяка, в жилую комнату у основания башни.",
        objective: getWakeObjective(),
      };
    },
  },
  {
    ...
    height: 56,
    title: "Лодка",
    text: "Это твоя рыбацкая лодка. Шторм едва не разнёс её в щепки, и в море на ней сейчас не уйти.",
    objective: "Цель: осмотреть берег и понять, кто принёс тебя в маяк после шторма.",
  },
  {
    id: "crate",
    x: 350,
  ...
  },
  {
    ...
    onInteract() {
      ...
      return {
        text: "Внутри мокрый плащ, пустой держатель ключей и жетон с инициалами кого-то из персонала маяка.",
        objective: getTunnelObjective(),
      };
    },
  },
      ...
      if (!gameState.puzzleState.tunnelExit.unlocked) {
        return {
          text: gameState.inventory.serviceKey
            ...
            : "Выход к пристани закрыт на старый служебный замок. Нужен ключ из шкафчика в тоннеле.",
          objective: getTunnelObjective(),
        };
      }
  ...
  },
  {
    ...
    onInteract() {
      ...
      return {
        text: "На мокрых камнях видны свежие следы и клочок ткани. Значит, кто-то был здесь совсем недавно и ушёл к северной бухте.",
        objective: "Новая цель: сделать следующую сцену у северной бухты и решить, кто именно оставил эти следы.",
      };
    },
  },
];
